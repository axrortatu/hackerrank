...
public class BotDifficultyLanguange {
    ...
    public List<Object> getDifficultyType(Long CHAT_ID, int topicId) {
        ...
        if (botLang.getLanguageName(CHAT_ID).equals(ENG)) {
            difficultyLang.addAll(List.of(EASY_ENG, MEDIUM_ENG, HARD_ENG, ALL_ENG,SOLVED,UNSOLVED));
        } else if (botLang.getLanguageName(CHAT_ID).equals(UZB)) {
            difficultyLang.addAll(List.of(EASY_UZB, MEDIUM_UZB, HARD_UZB, ALL_UZB,SOLVED,UNSOLVED));

        } else {
            difficultyLang.addAll(List.of(EASY_RUS, MEDIUM_RUS, HARD_RUS, ALL_RUS,SOLVED,UNSOLVED));
        }
